...
def repeat(im:Image, row: int = 0, column: int = 0, size: list|tuple = []) -> Image:
    if size:
...
def transparentImg(width: int = 0, height: int = 0, size: list|tuple = [], back: int = 255) -> Image:
    if size:
        width  = size[0]
        height = size[1]
...
def alphaPaste(back: Image, front: Image, point: list|tuple, changeSize:bool = False) -> Image:
    if changeSize:
