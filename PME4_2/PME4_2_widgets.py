...
import PME4_2_commands as commands
...
class PMEwidgets(commands.PMEcommands):
    ...
    def menubar(self):
        ...
        #ツール
        menu_tool = tke.MenuE(menu_bar, tearoff=False)
        #menu_tool.add_command(command = self.key_g, accelerator = "G", event = "<g>", label = "GDW用素材合成ツール")
        menu_bar.add_cascade(label="ツール", menu = menu_tool)
    ...
    def left_settings(self):
        ...
        self.fileLabel = tke.Label(self.LSframe.scrollable_frame, text = self.currentFile, wraplength=200)
        ...
        rb0 = ttk.Radiobutton(self.pasteSettingsframe, text="そのまま", value="no", variable=self.pasteSettings, command= lambda: self.pasteSettingsChanged("no"))
