...
import PME4_2_tkplus as tke
from PIL import ImageTk
from PIL.ImageTk import PhotoImage
import PME4_2_base    as base
import PME4_2_Imgplus as Image
...
class Resize(object):
    def __init__(self, container, nowImg: Image.Image = None):
        ...
        button = tke.ButtonE(self.toplevel, text ="開始", width = 15, command = self.toplevel.destroy).grid(row = 4, column = 0, columnspan = 2)
        labelframe = tke.Labelframe(self.toplevel, text = "補間").grid(row = 3, column = 0, columnspan = 2)
        nearest = tke.RadiobuttonE(labelframe, text = "最近傍(ドット絵向け)", value = "nearest", variable = self.resampling).pack(anchor = tk.W)
        bilinear = tke.RadiobuttonE(labelframe, text = "バイリニア", value = "bilinear", variable = self.resampling).pack(anchor = tk.W)
...
class GDWtool(SubWindows):
    def __init__(self, container, nowImg: Image.Image = None, nowCrop: Image.Image = None):
        ...
        self.top     :Image.Image = None
        self.side    :Image.Image = None
        self.bottom  :Image.Image = None
        self.topTK   :PhotoImage  = None
        self.sideTK  :PhotoImage  = None
        self.bottomTK:PhotoImage  = None

        labelTop    = tke.Label(self.toplevel, text = "上面", compound='right').grid(row = 0, column = 0)
        labelSide   = tke.Label(self.toplevel, text = "側面", compound='right').grid(row = 1, column = 0)
        labelBottom = tke.Label(self.toplevel, text = "底面", compound='right').grid(row = 2, column = 0)
        buttonAuto = tke.ButtonE(self.toplevel, text = "オートブロックを作る").grid(row = 1, column = 1)
        self.toplevel.drop_target_register(DND_FILES)
        self.toplevel.dnd_bind("<<Drop>>", self.dropped)
